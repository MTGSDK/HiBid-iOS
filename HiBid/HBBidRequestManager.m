...
- (void)getBidNetworks:(NSArray *)networkItems unitId:(NSString *)unitId adFormat:(HBAdBidFormat)format maxTimeoutMS:(NSInteger)maxTimeoutMS responseCallback:(void(^)(HBAuctionResult *auctionResponse,NSError *error))callback{
    ...
    [self requestNetworkBids:networkItems];
...
- (void)requestNetworkBids:(NSArray *)networkItems{
...
- (void)timerFireMethod:(NSTimer *)timer{
    ...
    for (HBBidNetworkItem *item  in self.networkItems) {
        ...
        if (![currentResponseDict objectForKey:key]) {
            ...
            NSString *errorMsg = [NSString stringWithFormat:@"Current network(%@) bid request timeout",[item getNetworkName]];
            NSError *error = [HBAdBidError errorWithCode:(HBBidErrorNetworkBidTimeout) userInfo:@{NSLocalizedDescriptionKey : errorMsg}];
